...
// v1 默认用 bio 字段渲染；若 content/about.md 存在则走管线渲染（见 utils/content.js 说明）
...
const aboutHtml = ref('')
const hasAboutFile = import.meta.glob('@/content/about.md', { query: '?raw', import: 'default', eager: true })

onMounted(async () => {
  // about.md 为可选文件（04 §3）：存在则渲染正文，否则用 bio
  const loader = Object.values(hasAboutFile)[0]
  if (loader) aboutHtml.value = renderMarkdown(loader, 'about')
})
