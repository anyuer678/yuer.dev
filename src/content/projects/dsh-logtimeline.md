...
| 纪律 | 做法 |
|------|------|
| 核心可测 | `query` 与 DSH `runtime` 装配分离 |
| 供应链 | `VENDORED.md` 记录上游 commit、唯一改动与同步验证 |
| 输出契约 | JSON 字段稳定，Agent 可直接消费 |
| 防护 | max_lines、超时、编码回退、输出上限 |
| 生态 | 接入 DeepSeek Harness 插件体系（公开 star） |

### 与作品集其它仓的关系

从 `logtimeline`（上游工具）→ 插件化再包装 → 上游归档，体现了「工具 → 产品化接口」的完整生命周期。

### 一句话

全账号里最像「可维护开源组件」的仓：**文档、契约、测试与 vendored 纪律都在**。
